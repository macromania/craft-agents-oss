...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
  ...
  const { options: sessionOpts, setOption, setPermissionMode } = useSessionOptionsFor(sessionId);
  ...
  const hasUnreadMessages = sessionMeta ? !!(sessionMeta.lastFinalMessageId && sessionMeta.lastFinalMessageId !== sessionMeta.lastReadMessageId) : false;
  ...
  const handleTodoStateChange = React.useCallback(
    (state: string) => {
    ...
    if (sessionMeta) {
      ...
      return (
        <>
          ...
          <RenameDialog open={renameDialogOpen} onOpenChange={setRenameDialogOpen} title="Rename Chat" value={renameName} onValueChange={setRenameName} onSubmit={handleRenameSubmit} placeholder="Enter chat name..." />
        </>
      );
    }
