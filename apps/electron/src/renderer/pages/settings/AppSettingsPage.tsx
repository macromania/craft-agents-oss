...
import { SettingsSection, SettingsCard, SettingsRow, SettingsToggle, SettingsSegmentedControl, SettingsMenuSelect } from "@/components/settings";
...
export default function AppSettingsPage() {
  ...
  const loadConnectionInfo = useCallback(async () => {
    ...
    try {
      const [billing, notificationsOn] = await Promise.all([window.electronAPI.getApiSetup(), window.electronAPI.getNotificationsEnabled()]);
          ...
          <div className="px-5 py-7 max-w-3xl mx-auto">
            <div className="space-y-8">
              ...
              <SettingsSection title="Notifications">
                <SettingsCard>
                  <SettingsToggle label="Desktop notifications" description="Get notified when AI finishes working in a chat." checked={notificationsEnabled} onCheckedChange={handleNotificationsEnabledChange} />
                </SettingsCard>
              </SettingsSection>
              ...
              <FullscreenOverlayBase isOpen={showApiSetup} onClose={closeApiSetup} className="z-splash flex flex-col bg-foreground-2">
                <OnboardingWizard
                  ...
                  // Copilot CLI props
                  copilotStatus={apiSetupOnboarding.copilotStatus}
                  onCopilotInstallClick={apiSetupOnboarding.handleCopilotInstallClick}
                  onCopilotAuthenticateClick={apiSetupOnboarding.handleCopilotAuthenticateClick}
                  onCopilotRefreshClick={apiSetupOnboarding.handleCopilotRefreshClick}
                  onCopilotContinue={apiSetupOnboarding.handleCopilotContinue}
                  className="h-full"
                />
                {/* Close button — rendered AFTER the wizard so it paints above its titlebar-drag-region */}
                <div className="fixed top-0 right-0 h-[50px] flex items-center pr-5 [-webkit-app-region:no-drag]" style={{ zIndex: "var(--z-fullscreen, 350)" }}>
                  <button
                ...
                <SettingsCard>
                  <SettingsRow label="Version">
                    <div className="flex items-center gap-2">
                      <span className="text-muted-foreground">{updateChecker.updateInfo?.currentVersion ?? "Loading..."}</span>
                      {updateChecker.updateAvailable && updateChecker.updateInfo?.latestVersion && (
                        <Button variant="outline" size="sm" onClick={updateChecker.installUpdate}>
