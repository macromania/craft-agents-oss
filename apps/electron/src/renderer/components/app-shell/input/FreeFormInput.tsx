...
import { Paperclip, ArrowUp, Square, Check, DatabaseZap, ChevronDown, Loader2 } from "lucide-react";
import { Icon_Home, Icon_Folder } from "@craft-agent/ui";

import * as storage from "@/lib/local-storage";

import { Button } from "@/components/ui/button";
import { InlineSlashCommand, useInlineSlashCommand, type SlashCommandId } from "@/components/ui/slash-command-menu";
import { InlineMentionMenu, useInlineMention, type MentionItem, type MentionItemType } from "@/components/ui/mention-menu";
import { InlineLabelMenu, useInlineLabelMenu } from "@/components/ui/label-menu";
import type { LabelConfig } from "@craft-agent/shared/labels";
import { parseMentions } from "@/lib/mentions";
import { RichTextInput, type RichTextInputHandle } from "@/components/ui/rich-text-input";
import { Tooltip, TooltipContent, TooltipTrigger } from "@craft-agent/ui";
import { DropdownMenu, DropdownMenuTrigger, DropdownMenuSub, DropdownMenuPortal } from "@/components/ui/dropdown-menu";
import { StyledDropdownMenuContent, StyledDropdownMenuItem, StyledDropdownMenuSeparator, StyledDropdownMenuSubTrigger, StyledDropdownMenuSubContent } from "@/components/ui/styled-dropdown";
import { Popover, PopoverTrigger, PopoverContent } from "@/components/ui/popover";
import { cn } from "@/lib/utils";
import { PATH_SEP, getPathBasename } from "@/lib/platform";
import { applySmartTypography } from "@/lib/smart-typography";
import { AttachmentPreview } from "../AttachmentPreview";
import { MODELS, COPILOT_MODELS, getModelShortName, getModelContextWindow, isClaudeModel } from "@config/models";
...
export function FreeFormInput({
  ...
  React.useEffect(() => {
    ...
    const changed = enabledSourceSlugs.length !== prev.length || enabledSourceSlugs.some((slug, i) => slug !== prev[i]);
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      >
        ...
        {/* Attachment Preview */}
        <AttachmentPreview attachments={attachments} onRemove={handleRemoveAttachment} disabled={disabled} loadingCount={loadingCount} />
        ...
        <div className="relative">
          ...
          <div className="flex items-center gap-1 px-2 py-2 border-t border-border/50">
            ...
            <FreeFormInputContextBadge
              ...
              label={attachments.length > 0 ? (attachments.length === 1 ? "1 file" : `${attachments.length} files`) : "Attach Files"}
                          ...
                          return (
                            <>
                              {displaySources.map((source, index) => (
                                <div key={source.config.slug} className={cn("relative h-5 w-5 rounded-[4px] bg-background shadow-minimal flex items-center justify-center", index > 0 && "-ml-1")} style={{ zIndex: index + 1 }}>
                                  <SourceAvatar source={source} size="xs" />
                                </div>
                              ))}
                              {remainingCount > 0 && (
                                <div className="-ml-1 h-5 w-5 rounded-[4px] bg-background shadow-minimal flex items-center justify-center text-[8px] font-medium text-muted-foreground" style={{ zIndex: displaySources.length + 1 }}>
                                  +{remainingCount}
                                </div>
                              )}
                            </>
                                ...
                                .map((source, index) => {
                                  ...
                                  return (
                                    <CommandPrimitive.Item
                                      ...
                                      onSelect={() => {
                                        const newSlugs = isEnabled ? optimisticSourceSlugs.filter((slug) => slug !== source.config.slug) : [...optimisticSourceSlugs, source.config.slug];
                                        // Optimistic update - UI updates immediately
                                        setOptimisticSourceSlugs(newSlugs);
                                        // Then trigger async server update
                                        onSourcesChange?.(newSlugs);
                                      }}
                                      className={cn("flex cursor-pointer select-none items-center gap-3 rounded-[6px] px-3 py-2 text-[13px]", "outline-none data-[selected=true]:bg-foreground/5", isEnabled && "bg-foreground/3")}
                                    >
                                      ...
                                      <div className={cn("shrink-0 h-4 w-4 rounded-full bg-current flex items-center justify-center", !isEnabled && "opacity-0")}>
            ...
            {onWorkingDirectoryChange && <WorkingDirectoryBadge workingDirectory={workingDirectory} onWorkingDirectoryChange={onWorkingDirectoryChange} sessionFolderPath={sessionFolderPath} isEmptySession={isEmptySession} />}
            ...
            <DropdownMenu open={modelDropdownOpen} onOpenChange={setModelDropdownOpen}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <DropdownMenuTrigger asChild>
                    <button type="button" className={cn("inline-flex items-center h-7 px-1.5 gap-0.5 text-[13px] shrink-0 rounded-[6px] hover:bg-foreground/5 transition-colors select-none", modelDropdownOpen && "bg-foreground/5")}>
              ...
              <StyledDropdownMenuContent side="top" align="end" sideOffset={8} className="min-w-[240px]">
                ...
                {customModel ? (
                  ...
                  (provider === "copilot" ? COPILOT_MODELS : MODELS).map((model) => {
                    ...
                    const descriptions: Record<string, string> = {
                      ...
                      "gpt-4.1": "Balanced for everyday tasks",
                      "claude-sonnet-4.5": "Claude via Copilot",
                      "claude-haiku-4.5": "Fast Claude via Copilot",
                    };
                    return (
                      <StyledDropdownMenuItem key={model.id} onSelect={() => onModelChange(model.id)} className="flex items-center justify-between px-2 py-2 rounded-lg cursor-pointer">
                        <div className="text-left">
                ...
                {(!customModel || isClaudeModel(customModel)) && provider !== "copilot" && (
                  <>
                    ...
                    <DropdownMenuSub>
                      ...
                      <StyledDropdownMenuSubContent className="min-w-[220px]">
                        {THINKING_LEVELS.map(({ id, name, description }) => {
                          ...
                          return (
                            <StyledDropdownMenuItem key={id} onSelect={() => onThinkingLevelChange?.(id)} className="flex items-center justify-between px-2 py-2 rounded-lg cursor-pointer">
                              <div className="text-left">
                    ...
                    <div className="px-2 py-1.5 select-none">
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1.5">
                          {contextStatus.isCompacting && <Loader2 className="h-3 w-3 animate-spin" />}
                          ...
                          {(() => {
                            ...
                            return ctxWindow ? <span className="opacity-60">/ {formatTokenCount(Math.round(ctxWindow * 0.775))}</span> : null;
                          })()}
                        </span>
                      </div>
                    </div>
            ...
            {(() => {
              ...
              const usagePercent = contextStatus?.inputTokens && compactionThreshold ? Math.min(99, Math.round((contextStatus.inputTokens / compactionThreshold) * 100)) : null;
              ...
              return (
                <Tooltip>
                  ...
                  <TooltipContent side="top">{isProcessing ? `${usagePercent}% context used — wait for current operation` : `${usagePercent}% context used — click to compact`}</TooltipContent>
                </Tooltip>
              );
            })()}
            ...
            {isProcessing ? (
              <Button type="button" size="icon" variant="secondary" className="h-7 w-7 rounded-full shrink-0 hover:bg-foreground/15 active:bg-foreground/20 ml-2" onClick={() => handleStop(false)}>
                <Square className="h-3 w-3 fill-current" />
              </Button>
            ) : (
              <Button type="submit" size="icon" className="h-7 w-7 rounded-full shrink-0 ml-2" disabled={!hasContent || disabled} data-tutorial="send-button">
...
function formatPathForDisplay(path: string, homeDir: string): string {
  ...
  if (homeDir && path.startsWith(homeDir)) {
    ...
    displayPath = relativePath.startsWith(PATH_SEP) ? relativePath.slice(1) : relativePath || PATH_SEP;
  }
      ...
      <PopoverContent side="top" align="start" sideOffset={8} className={MENU_CONTAINER_STYLE}>
        <CommandPrimitive shouldFilter={showFilter}>
          ...
          <CommandPrimitive.List className={MENU_LIST_STYLE}>
            ...
            {hasFolder && (
              <CommandPrimitive.Item value={`current-${workingDirectory}`} className={cn(MENU_ITEM_STYLE, "pointer-events-none bg-foreground/5")} disabled>
            ...
            {hasFolder && filteredRecent.length > 0 && <div className="h-px bg-border my-1 mx-1" />}
            ...
            {filteredRecent.map((path) => {
              ...
              return (
                <CommandPrimitive.Item key={path} value={`${recentFolderName} ${path}`} onSelect={() => handleSelectRecent(path)} className={cn(MENU_ITEM_STYLE, "data-[selected=true]:bg-foreground/5")}>
                  <Icon_Folder className="h-4 w-4 shrink-0 text-muted-foreground" />
                  <span className="flex-1 min-w-0 truncate">
            ...
            {showFilter && <CommandPrimitive.Empty className="py-3 text-center text-sm text-muted-foreground">No folders found</CommandPrimitive.Empty>}
          </CommandPrimitive.List>

          {/* Bottom actions - always visible, outside scrollable area */}
          <div className="border-t border-border/50 p-1">
            <button type="button" onClick={handleChooseFolder} className={cn(MENU_ITEM_STYLE, "w-full hover:bg-foreground/5")}>
              Choose Folder...
            </button>
            {showReset && (
              <button type="button" onClick={handleReset} className={cn(MENU_ITEM_STYLE, "w-full hover:bg-foreground/5")}>
                Reset
              </button>
            )}
          </div>
