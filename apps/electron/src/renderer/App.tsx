...
import { initializeSessionsAtom, addSessionAtom, removeSessionAtom, updateSessionAtom, sessionAtomFamily, sessionMetaMapAtom, backgroundTasksAtomFamily, extractSessionMeta, type SessionMeta } from "@/atoms/sessions";
 ...
 */
function handleBackgroundTaskEvent(store: JotaiStore, sessionId: string, event: { type: string }, agentEvent: unknown): void {
  ...
  if (event.type === "task_backgrounded" && "taskId" in evt && "toolUseId" in evt) {
    ...
    store.set(
      backgroundTasksAtom,
      currentTasks.map((t) => (t.toolUseId === evt.toolUseId ? { ...t, elapsedSeconds: evt.elapsedSeconds as number } : t))
    );
    ...
    store.set(
      backgroundTasksAtom,
      currentTasks.filter((t) => t.id !== evt.shellId)
    );
    ...
    const isBackgroundingResult = result && (/agentId:\s*[a-zA-Z0-9_-]+/.test(result) || /shell_id:\s*[a-zA-Z0-9_-]+/.test(result) || /"backgroundTaskId":\s*"[a-zA-Z0-9_-]+"/.test(result));
    if (!isBackgroundingResult) {
      const currentTasks = store.get(backgroundTasksAtom);
      store.set(
        backgroundTasksAtom,
        currentTasks.filter((t) => t.toolUseId !== evt.toolUseId)
      );
    }
  }
...
export default function App() {
  ...
  const updateSessionById = useCallback(
    (sessionId: string, updates: Partial<Session> | ((session: Session) => Partial<Session>)) => {
    ...
    const cleanup = window.electronAPI.onSessionEvent((event: SessionEvent) => {
      ...
      if (event.type === "info" && event.statusType === "compaction_complete") {
        window.dispatchEvent(
          new CustomEvent("craft:compaction-complete", {
            detail: { sessionId },
          })
        );
      }
      ...
      if (isStreaming || isHandoff) {
        ...
        // Process the event
        const { session: updatedSession, effects } = processAgentEvent(agentEvent, currentSession, workspaceId);
        ...
        if (isHandoff) {
          ...
          if (event.type === "complete") {
            ...
            const lastMessage = updatedSession.messages.findLast((m) => m.role === "assistant" && !m.isIntermediate);
  ...
  const handleCreateSession = useCallback(
    async (workspaceId: string, options?: import("../shared/types").CreateSessionOptions): Promise<Session> => {
  ...
  const handleDeleteSession = useCallback(
    async (sessionId: string, skipConfirmation = false): Promise<boolean> => {
  ...
  const handleUnflagSession = useCallback(
    (sessionId: string) => {
   ...
   */
  const handleSetActiveViewingSession = useCallback(
    (sessionId: string) => {
  ...
  const handleMarkSessionRead = useCallback(
    (sessionId: string) => {
      ...
      updateSessionById(sessionId, (s) => {
        const lastFinalId = s.messages.findLast((m) => m.role === "assistant" && !m.isIntermediate)?.id;
        return {
          hasUnread: false,
  ...
  const handleTodoStateChange = useCallback(
    (sessionId: string, state: TodoState) => {
  ...
  const handleSendMessage = useCallback(
    async (sessionId: string, message: string, attachments?: FileAttachment[], skillSlugs?: string[]) => {
      try {
        ...
        if (attachments?.length) {
          ...
          const storeResults = await Promise.allSettled(attachments.map((a) => window.electronAPI.storeAttachment(sessionId, a)));
        ...
        const badges: ContentBadge[] = windowWorkspaceId ? extractBadges(message, skills, sources, windowWorkspaceId) : [];
   ...
   */
  const handleSessionOptionsChange = useCallback(
    (sessionId: string, updates: SessionOptionUpdates) => {
      setSessionOptions((prev) => {
        const next = new Map(prev);
  ...
  const openNewChat = useCallback(
    async (params: NewChatActionParams = {}) => {
  ...
  const handleSelectWorkspace = useCallback(
    async (workspaceId: string, openInNewWindow = false) => {
  ...
  if (appState === "reauth") {
    return (
      <ModalProvider>
        <WindowCloseHandler />
        <ReauthScreen onLogin={handleReauthLogin} onReset={handleReauthReset} />
        <ResetConfirmationDialog open={showResetDialog} onConfirm={executeReset} onCancel={() => setShowResetDialog(false)} />
      </ModalProvider>
    );
  }
    ...
    <PlatformProvider actions={platformActions}>
      <ShikiThemeProvider shikiTheme={shikiTheme}>
        <FocusProvider>
          <ModalProvider>
            <TooltipProvider>
              <NavigationProvider workspaceId={windowWorkspaceId} onCreateSession={handleCreateSession} onInputChange={handleInputChange} isReady={appState === "ready"}>
                {/* Handle window close requests (X button, Cmd+W) - close modal first if open */}
                <WindowCloseHandler />

                {/* Splash screen overlay - fades out when fully ready */}
                {showSplash && <SplashScreen isExiting={splashExiting} onExitComplete={handleSplashExitComplete} />}

                {/* Main UI - always rendered, splash fades away to reveal it */}
                <div className="h-full flex flex-col text-foreground">
                  <div className="flex-1 min-h-0">
                    <AppShell contextValue={appShellContextValue} defaultLayout={[20, 32, 48]} menuNewChatTrigger={menuNewChatTrigger} isFocusedMode={isFocusedMode} />
                  </div>
                  <ResetConfirmationDialog open={showResetDialog} onConfirm={executeReset} onCancel={() => setShowResetDialog(false)} />
                </div>
              </NavigationProvider>
