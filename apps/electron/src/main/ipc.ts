...
async function validateFilePath(filePath: string): Promise<string> {
  ...
  const sensitivePatterns = [/\.ssh\//, /\.gnupg\//, /\.aws\/credentials/, /\.env$/, /\.env\./, /credentials\.json$/, /secrets?\./i, /\.pem$/, /\.key$/];
...
export function registerIpcHandlers(sessionManager: SessionManager, windowManager: WindowManager): void {
  ...
  ipcMain.handle(IPC_CHANNELS.SEND_MESSAGE, async (event, sessionId: string, message: string, attachments?: FileAttachment[], storedAttachments?: StoredAttachment[], options?: SendMessageOptions) => {
    ...
    sessionManager.sendMessage(sessionId, message, attachments, storedAttachments, options).catch((err) => {
      ...
      const window = callingWorkspaceId ? windowManager.getWindowByWorkspace(callingWorkspaceId) : BrowserWindow.getFocusedWindow() || BrowserWindow.getAllWindows()[0];
  ...
  ipcMain.handle(IPC_CHANNELS.SESSION_COMMAND, async (_event, sessionId: string, command: import("../shared/types").SessionCommand) => {
    switch (command.type) {
      case "flag":
  ...
  ipcMain.handle(IPC_CHANNELS.SET_PROVIDER, async (_event, provider: import("@craft-agent/shared/agent/providers/types").ProviderType) => {
    const { setProvider } = await import("@craft-agent/shared/config");
    setProvider(provider);
    ipcLog.info(`[Provider] Switched to provider: ${provider}`);
    // Note: Session manager will pick up the new provider on next session creation
  });

  // Check Copilot CLI status (installed and authenticated)
  // Note: copilot CLI uses gh CLI for authentication, so we check:
  ...
  // Start GitHub authentication flow (required for Copilot CLI)
  // Uses gh auth login which handles the OAuth flow
  ipcMain.handle(IPC_CHANNELS.START_COPILOT_AUTH, async () => {
    try {
      // Run gh auth login which opens browser for GitHub OAuth
      // Use spawn for interactive auth flow instead of execSync
      ...
      return new Promise((resolve) => {
        ...
        proc.on("close", (code) => {
          if (code === 0) {
            ...
            resolve({ success: false, error: `gh auth login exited with code ${code}` });
          }
        });
        proc.on("error", (err) => {
  ...
  ipcMain.handle(IPC_CHANNELS.FS_SEARCH, async (_event, basePath: string, query: string) => {
    ...
    const SKIP_DIRS = new Set(["node_modules", ".git", ".svn", ".hg", "dist", "build", ".next", ".nuxt", ".cache", "__pycache__", "vendor", ".idea", ".vscode", "coverage", ".nyc_output", ".turbo", "out"]);
  ...
  ipcMain.handle(IPC_CHANNELS.SETTINGS_TEST_API_CONNECTION, async (_event, apiKey: string, baseUrl?: string, modelName?: string): Promise<{ success: boolean; error?: string; modelCount?: number }> => {
    ...
    try {
      ...
      const isModelNotFound = lowerMsg.includes("model not found") || lowerMsg.includes("is not a valid model") || lowerMsg.includes("invalid model") || (lowerMsg.includes("404") && lowerMsg.includes("model"));
      if (isModelNotFound) {
        if (modelName?.trim()) {
    ...
    try {
      ...
      if (source.config.mcp.transport === "stdio") {
        ...
        if (source.config.mcp.authType === "oauth" || source.config.mcp.authType === "bearer") {
          ...
          const credentialId =
            source.config.mcp.authType === "oauth" ? { type: "source_oauth" as const, workspaceId: source.workspaceId, sourceId: sourceSlug } : { type: "source_bearer" as const, workspaceId: source.workspaceId, sourceId: sourceSlug };
  ...
  ipcMain.handle(IPC_CHANNELS.THEME_BROADCAST_PREFERENCES, async (event, preferences: { mode: string; colorTheme: string; font: string }) => {
    ...
    for (const managed of windowManager.getAllWindows()) {
      if (!managed.window.isDestroyed() && !managed.window.webContents.isDestroyed() && managed.window.webContents.mainFrame && managed.window.webContents.id !== senderId) {
