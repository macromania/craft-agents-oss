...
import { IPC_CHANNELS, type OnboardingSaveResult } from "../shared/types";
    ...
    async (
      ...
      try {
        ...
        if (config.authType) {
          ...
          // 3.1 Set provider based on authType
          // copilot_cli uses the 'copilot' provider, all others use 'claude'
          const newProvider = config.authType === "copilot_cli" ? "copilot" : "claude";
          mainLog.info("[Onboarding:Main] Setting provider to", newProvider, "based on authType", config.authType);
